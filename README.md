...
|:--------------------------|:---------|:-------------------------|:----------------------|
| `basePath`                | `string` | Path to the OIDC-Compatible Identity Provider. Will be used as the baseUrl for the `authorizationEndpoint`, `revocationEndpoint` and `endSessionEndpoint` | *none*
...
| `apiUrl`                  | `string` | The url to your backend which should be protected by adding the token for outgoing requests. | `/api/`
